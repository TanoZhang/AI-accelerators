...
if {![info exists ::env(DE25_USE_PARALLEL_FEEDER)]
    || $::env(DE25_USE_PARALLEL_FEEDER) ni {0 1}} {
  error "Set DE25_USE_PARALLEL_FEEDER to 0 or 1."
}
...
if {[file exists "${project_path}.qpf"]} {
  project_open $project_path
} else {
  project_new $project_path
}
...
set_parameter -name USE_PARALLEL_FEEDER $::env(DE25_USE_PARALLEL_FEEDER)
...
set_global_assignment -name SYSTEMVERILOG_FILE [file join $repo_root rtl operand_feeder.sv]
